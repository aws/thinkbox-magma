...
class magma_intersection_node : public magma_simple_operator<4> {
  public:
    struct meta {
        enum { ARITY = 3 };
        typedef frantic::magma::functors::intersect_ray type;
        typedef boost::mpl::vector<void( void*, vec3, vec3, int ), void( void*, vec3, vec3, float ),
                                   void( void*, vec3, vec3, bool )>
            bindings;
    };

    MAGMA_REQUIRED_METHODS( magma_intersection_node );
    ...
    class functor {
      ...
      public:
        ...
        typedef boost::mpl::vector<frantic::graphics::vector3f, frantic::graphics::vector3f> param_types;
      ...
      public:
        ...
        bool operator()( const frantic::graphics::vector3f& pos, const frantic::graphics::vector3f& dir,
            ...
            try {
                ...
                for( std::vector<magma_geometry_ptr>::const_iterator it = m_meshes.begin(), itEnd = m_meshes.end();
                     it != itEnd; ++it, ++curPrim ) {
                    bool isValid = ( *it )->intersect_ray( pos, dir, outResult.distance, outResult );
                    if( isValid ) {
                        result = true;
        ...
        inline void operator()( void* _out, const frantic::graphics::vector3f& pos,
                                const frantic::graphics::vector3f& dir ) const throw() {
