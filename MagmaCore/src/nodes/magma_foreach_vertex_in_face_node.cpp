...
// MAGMA_DEFINE_TYPE( "VertexLoop", "System", magma_foreach_vertex_in_face_node )
namespace {
frantic::magma::magma_node_base* create_magma_foreach_vertex_in_face_node() {
    return new magma_foreach_vertex_in_face_node;
}
}
void magma_foreach_vertex_in_face_node::compile(
    frantic::magma::magma_compiler_interface& compiler ) { // compiler.compile( this );
    this->compile_as_extension_type( compiler );
}
void magma_foreach_vertex_in_face_node::create_type_definition( frantic::magma::magma_node_type& _outType ) {
    frantic::magma::magma_node_type_impl& outType = dynamic_cast<frantic::magma::magma_node_type_impl&>( _outType );
    outType.set_name( _T( "VertexLoopByFace" ) );
    outType.set_category( _T( "Loop") );
    outType.set_factory( &create_magma_foreach_vertex_in_face_node );
    {
    ...
    typedef magma_foreach_vertex_in_face_node this_type;
    MAGMA_DESCRIPTION( "Runs an operation over all vertices in a face." )
    ...
    MAGMA_DEFINE_TYPE( "VertexLoopByFace__Input", "System", magma_foreach_vertex_in_face_inputs_node )
    MAGMA_TYPE_ATTR( public, false );
    MAGMA_DEFINE_TYPE_END

    MAGMA_DEFINE_TYPE( "VertexLoopByFace__Output", "System", magma_foreach_vertex_in_face_outputs_node )
    MAGMA_TYPE_ATTR( public, false );
    MAGMA_DEFINE_TYPE_END

    const std::vector<frantic::tstring>& magma_foreach_vertex_in_face_node::get_loopChannels() const {
        if( m_loopChannels
            ...
            this->get_type().get_singleton().get_predefined_face_vertex_channels( vertChannels );
    ...
    magma_foreach_vertex_in_face_node::magma_foreach_vertex_in_face_node()
        : m_geomInput( magma_interface::INVALID_ID, 0 )
        , m_objIndexInput( magma_interface::INVALID_ID, 0 )
        , m_faceIndexInput( magma_interface::INVALID_ID, 0 )
        , m_geomDefault()
        , m_objIndexDefault( 0 )
        , m_vertIndexDefault( 0 ) {}

    void magma_foreach_vertex_in_face_node::do_clone( magma_node_base & dest, clone_callback & cb ) const {
        magma_loop_node::do_clone( dest, cb );
    }

    int magma_foreach_vertex_in_face_node::get_num_inputs() const {
        return magma_loop_node::get_num_inputs() + 3; // Add one input for the geometry
    }
    std::pair<magma_interface::magma_id, int> magma_foreach_vertex_in_face_node::get_input( int index ) const {
        if( index == 0 )
            return m_geomInput;
        else if( index == 1 )
            return m_objIndexInput;
        else if( index == 2 )
            return m_faceIndexInput;
        else
            return magma_loop_node::get_input( index - 3 );
    }

    void magma_foreach_vertex_in_face_node::get_input_description( int i, frantic::tstring& outDescription ) {
        if( i == 0 )
            outDescription = _T("Geometry");
        else if( i == 1 )
            outDescription = _T("ObjIndex");
        else if( i == 2 )
            outDescription = _T("FaceIndex");
        else
            magma_loop_node::get_input_description( i - 3, outDescription );
    }

    void magma_foreach_vertex_in_face_node::get_output_description( int i, frantic::tstring& outDescription ) const {
        const_cast<magma_foreach_vertex_in_face_node*>( this )->get_input_description( i + 3, outDescription );
    }

    void magma_foreach_vertex_in_face_node::set_input( int index, magma_interface::magma_id id, int socketIndex ) {
        if( index == 0 )
            m_geomInput = std::make_pair( id, socketIndex );
        else if( index == 1 )
            m_objIndexInput = std::make_pair( id, socketIndex );
        else if( index == 2 )
            m_faceIndexInput = std::make_pair( id, socketIndex );
        else
            magma_loop_node::set_input( index - 3, id, socketIndex );
    }

    const variant_t& magma_foreach_vertex_in_face_node::get_input_default_value( int index ) const {
        if( index == 0 )
    ...
    void magma_foreach_vertex_in_face_node::set_input_default_value( int index, const variant_t& value ) {
        if( index == 0 )
    ...
    magma_foreach_vertex_in_face_inputs_node::magma_foreach_vertex_in_face_inputs_node()
        : magma_loop_inputs_node( 3 ) {}

    magma_foreach_vertex_in_face_outputs_node::magma_foreach_vertex_in_face_outputs_node()
        : magma_loop_outputs_node( 0 ) {}

    using namespace frantic::magma::simple_compiler;

    class foreach_vertex_in_face_expression : public base_compiler::foreach_expression {
        std::vector<magma_geometry_ptr> m_geometry;
        std::ptrdiff_t m_controlInputs[2]; // Ptrs to input values (ObjIndex & FaceIndex)
      ...
      public:
        foreach_vertex_in_face_expression() {
            m_iterationPtr = -1;
            m_maxIterations = 1000;
        }

        virtual ~foreach_vertex_in_face_expression() {
        ...
        virtual const frantic::channels::channel_map& get_temporary_variables() const {
            static frantic::channels::channel_map s_emptyMap;
            if( !s_emptyMap.channel_definition_complete() )
                s_emptyMap.end_channel_definition();
            return s_emptyMap;
        }

        virtual void set_temporary_variables( std::ptrdiff_t /*tempPtr*/ ) {}
        ...
        virtual void handle_loop_input( base_compiler& compiler, base_compiler::expression_id exprID,
            ...
            typedef frantic::magma::functors::face_vertex_query functor_type;
            ...
            std::unique_ptr<base_compiler::expression> result(
                new frantic::magma::simple_compiler::detail::expression_impl<functor_type,
                                                                             void( void*, int, int, int )>( fn ) );

            result->set_input( 0, m_controlInputs[0] );
            result->set_input( 1, m_controlInputs[1] );
            result->set_input( 2, m_iterationPtr );
    ...
    void foreach_vertex_in_face_expression::apply( base_compiler::state & data ) const {
        ...
        std::size_t faceIndex = static_cast<std::size_t>( data.get_temporary<int>( m_controlInputs[1] ) );
        ...
        for( std::size_t i = 0, iEnd = theMesh.get_num_face_verts( static_cast<int>( faceIndex ) ); i < iEnd; ++i ) {
            data.set_temporary( m_iterationPtr, static_cast<int>( i ) );
    ...
    void magma_foreach_vertex_in_face_node::compile_as_extension_type( magma_compiler_interface & compiler ) {
        if( base_compiler* bc = dynamic_cast<base_compiler*>( &compiler ) ) {
            std::unique_ptr<foreach_vertex_in_face_expression> loopExpr( new foreach_vertex_in_face_expression );
