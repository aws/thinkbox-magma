...
// MAGMA_DEFINE_TYPE( "VertexLoop", "System", magma_foreach_face_node )
namespace {
frantic::magma::magma_node_base* create_magma_foreach_face_node() { return new magma_foreach_face_node; }
}
void magma_foreach_face_node::compile(
    frantic::magma::magma_compiler_interface& compiler ) { // compiler.compile( this );
    this->compile_as_extension_type( compiler );
}
void magma_foreach_face_node::create_type_definition( frantic::magma::magma_node_type& _outType ) {
    frantic::magma::magma_node_type_impl& outType = dynamic_cast<frantic::magma::magma_node_type_impl&>( _outType );
    outType.set_name( _T( "FaceLoopByEdge" ) );
    outType.set_category( _T( "Loop") );
    outType.set_factory( &create_magma_foreach_face_node );
    {
    ...
    typedef magma_foreach_face_node this_type;
    MAGMA_DESCRIPTION( "Runs an operation over all connected faces" )
    ...
    MAGMA_DEFINE_TYPE( "FaceLoopByEdge__Input", "System", magma_foreach_face_inputs_node )
    MAGMA_TYPE_ATTR( public, false );
    MAGMA_DEFINE_TYPE_END

    MAGMA_DEFINE_TYPE( "FaceLoopByEdge__Output", "System", magma_foreach_face_outputs_node )
    MAGMA_TYPE_ATTR( public, false );
    MAGMA_DEFINE_TYPE_END

    const std::vector<frantic::tstring>& magma_foreach_face_node::get_loopChannels() const {
        if( m_loopChannels
    ...
    magma_foreach_face_node::magma_foreach_face_node()
    ...
    void magma_foreach_face_node::do_clone( magma_node_base & dest, clone_callback & cb ) const {
        magma_loop_node::do_clone( dest, cb );
    }

    int magma_foreach_face_node::get_num_inputs() const {
        return magma_loop_node::get_num_inputs() + 3; // Add one input for the geometry
    }
    std::pair<magma_interface::magma_id, int> magma_foreach_face_node::get_input( int index ) const {
        if( index == 0 )
            return m_geomInput;
        else if( index == 1 )
    ...
    void magma_foreach_face_node::get_input_description( int i, frantic::tstring& outDescription ) {
        if( i == 0 )
    ...
    void magma_foreach_face_node::get_output_description( int i, frantic::tstring& outDescription ) const {
        const_cast<magma_foreach_face_node*>( this )->get_input_description( i + 3, outDescription );
    }

    void magma_foreach_face_node::set_input( int index, magma_interface::magma_id id, int socketIndex ) {
        if( index == 0 )
    ...
    const variant_t& magma_foreach_face_node::get_input_default_value( int index ) const {
        if( index == 0 )
    ...
    void magma_foreach_face_node::set_input_default_value( int index, const variant_t& value ) {
        if( index == 0 )
    ...
    magma_foreach_face_inputs_node::magma_foreach_face_inputs_node()
        : magma_loop_inputs_node( 3 ) {}

    magma_foreach_face_outputs_node::magma_foreach_face_outputs_node()
        : magma_loop_outputs_node( 0 ) {}

    using namespace frantic::magma::simple_compiler;

    class foreach_face_expression : public base_compiler::foreach_expression {
      ...
      public:
        foreach_face_expression() {
            m_iterationPtr = -1;
            m_maxIterations = 1000;
        }

        virtual ~foreach_face_expression() {
    ...
    void foreach_face_expression::apply( base_compiler::state & data ) const {
        ...
        theMesh.init_face_iterator( fIt, faceIndex );

        std::size_t counter = 0;
        do {
            std::size_t otherFace = theMesh.get_face_neighbor( fIt );
            if( otherFace != static_cast<std::size_t>( -1 ) ) {
                if( m_iterationPtr >= 0 )
                    data.set_temporary( m_iterationPtr, static_cast<int>( counter ) );

                data.set_temporary( m_neighborIndexPtr, static_cast<int>( otherFace ) );
            ...
            ++counter;
        } while( counter < m_maxIterations && theMesh.advance_face_iterator( fIt ) );
    }

    void magma_foreach_face_node::compile_as_extension_type( magma_compiler_interface & compiler ) {
        if( base_compiler* bc = dynamic_cast<base_compiler*>( &compiler ) ) {
            std::unique_ptr<foreach_face_expression> loopExpr( new foreach_face_expression );
